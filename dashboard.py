...
st.markdown("""
    <style>
        ...
        .stApp::before {
            ...
            pointer-events: none;
            z-index: 0;
            background: repeating-linear-gradient(0deg, transparent, transparent 2px, rgba(0, 212, 255, 0.03) 2px, rgba(0, 212, 255, 0.03) 4px), repeating-linear-gradient(90deg, transparent, transparent 2px, rgba(0, 212, 255, 0.03) 2px, rgba(0, 212, 255, 0.03) 4px);
        ...
        .stApp::after {
            ...
            pointer-events: none;
            z-index: 0;
            background: radial-gradient(circle at 20% 30%, rgba(0, 212, 255, 0.05) 1px, transparent 1px), radial-gradient(circle at 80% 70%, rgba(0, 212, 255, 0.05) 1px, transparent 1px), radial-gradient(circle at 50% 50%, rgba(0, 212, 255, 0.05) 1px, transparent 1px);
...
with tabs[3]:
    ...
    recommendations = {
        'rock': {'title': '✊ ROCK - Kekuatan & Ketegasan', 'items': [{'nama': 'Action Games', 'deskripsi': 'Game penuh aksi dan tantangan yang membutuhkan ketegasan.', 'kategori': 'Gaming'}, {'nama': 'Strength Training', 'deskripsi': 'Program latihan kekuatan untuk membangun otot.', 'kategori': 'Fitness'}]},
        'paper': {'title': '✋ PAPER - Fleksibilitas & Kreativitas', 'items': [{'nama': 'Creative Writing', 'deskripsi': 'Kursus menulis kreatif untuk mengembangkan imajinasi.', 'kategori': 'Education'}, {'nama': 'Digital Art', 'deskripsi': 'Belajar seni digital dan desain grafis.', 'kategori': 'Art'}]},
        'scissors': {'title': '✌ SCISSORS - Ketepatan & Strategi', 'items': [{'nama': 'Strategy Games', 'deskripsi': 'Game strategi yang melatih pemikiran taktis.', 'kategori': 'Gaming'}, {'nama': 'Chess Lessons', 'deskripsi': 'Pelajari strategi catur dari master.', 'kategori': 'Education'}]}
    }
    ...
    if current_classification in recommendations:
        ...
        with col1:
            ...
            for item in rec_data['items']:
                st.markdown(f"""
                <div class='menu-item'>
                    <span style='font-weight: bold; color: #00d4ff;'>{item['nama']}</span><br>
                    <span style='font-size: 0.85rem; color: #ffd700;'>[{item['kategori']}]</span><br>
                    <span style='font-size: 0.9rem;'>{item['deskripsi']}</span>
                </div>
                """, unsafe_allow_html=True)
        with col2:
            st.markdown("### 💡 Tips & Saran")
            tips = {'rock': ['Fokus pada kekuatan mental', 'Bangun ketahanan', 'Latih konsistensi'], 'paper': ['Eksplorasi ide baru', 'Berpikir out of the box', 'Fleksibel dalam pendekatan'], 'scissors': ['Rencanakan strategi', 'Analisa sebelum bertindak', 'Fokus pada detail']}
            for tip in tips.get(current_classification, []):
                st.markdown(f"""<div class='menu-item'><span style='font-size: 1rem;'>💡 {tip}</span></div>""", unsafe_allow_html=True)
    else:
        st.markdown("""
        <div class='recommendation-alert'>
            <p><span style='font-size: 2rem;'>✊✋✌</span><br>
            <span style='font-weight: bold;'>Silakan lakukan Klasifikasi Gesture terlebih dahulu</span> untuk mendapatkan rekomendasi personal.</p>
        </div>
        """, unsafe_allow_html=True)
